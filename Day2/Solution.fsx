...
let isPolicyValidForPart1 policy =
    let charOccurrence = policy.Password.Count(fun x -> x = policy.Letter)
    charOccurrence <= policy.SecondNumber && charOccurrence >= policy.FirstNumber
    
let isPolicyValidForPart2 policy =
    match (policy.Password.[policy.FirstNumber - 1] = policy.Letter, policy.Password.[policy.SecondNumber - 1] = policy.Letter) with
    | (true, false) -> true
    | (false, true) -> true
    | _ -> false
